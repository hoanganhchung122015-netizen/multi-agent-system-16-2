...
import { 
  Camera, Image as ImageIcon, MessageSquare, History, 
...
const App: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    setLoading(true);
    try {
      const optimizedImg = image ? await optimizeImage(image) : undefined;
      const data = await fetchFullAnalysis(subject, input, optimizedImg);
      ...
      <div className="bg-slate-50 rounded-[3rem] p-8 space-y-6 border border-slate-100">
        ...
        <button
          ...
          className={`w-full py-6 rounded-[2rem] font-black uppercase tracking-[0.2em] text-sm flex items-center justify-center gap-3 transition-all ${
            loading ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white shadow-xl shadow-indigo-200 hover:scale-[1.02] active:scale-95'
          }`}
        >
          {loading ? (
          ...
          <article className="prose prose-slate max-w-none prose-headings:font-black prose-headings:uppercase prose-headings:tracking-widest prose-headings:text-xs">
            {activeAgent === AgentType.SPEED && (
              <div className="space-y-6">
                ...
                {results.prof1.casio && (
                  <section>
                    <h4>Bấm máy CASIO 580VNX</h4>
                    <div className="bg-slate-900 text-emerald-400 p-6 rounded-3xl font-mono text-sm leading-relaxed border-l-4 border-emerald-500">
                      {results.prof1.casio}
                    </div>
                  </section>
                )}
            ...
            {activeAgent === AgentType.PERPLEXITY && (
              <div className="space-y-8">
                <h4>Câu hỏi luyện tập tương tự</h4>
                  ...
                  <div key={i} className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4">
                    ...
                    <details className="group">
                      ...
                      <div className="mt-4 p-4 bg-emerald-50 rounded-2xl text-emerald-700 text-sm">
                        <span className="font-black">Đáp án: {q.answer}</span>. {q.explanation}
                      </div>
                    </details>
                  </div>
                ))}
  ...
  const renderDiary = () => (
    <div className="space-y-4 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { if(confirm('Xóa hết lịch sử?')) { setDiary([]); localStorage.removeItem('symbiotic_diary'); }}} className="text-xs font-bold text-rose-500 uppercase tracking-widest">Xóa tất cả</button>
      </div>
      {diary.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200">
          <History size={48} className="mx-auto text-slate-200 mb-4" />
          <p className="text-slate-400 font-medium">Chưa có bài giải nào được lưu.</p>
        </div>
      ) : (
        diary.map((entry, i) => (
          <button
          ...
          >
            {entry.image && <img src={entry.image} className="w-16 h-16 rounded-2xl object-cover" />}
            <div className="flex-1">
              <div className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1">{entry.subject} • {entry.date}</div>
  ...
  return (
    ...
    >
      ...
      {showScanner && (
        <CameraScanner 
          onCapture={(data) => { setImage(data); setShowScanner(false); setView('INPUT'); }} 
